...
public class SeatAllocationReport {
    public static void allocationReport(){

        System.out.println(numberOfCandidates + " candidates took part in the elections from " + numberOfElectionCommittees + " election committees");
        ...
        for(int i=0; i < 6; i++){
            for(int j=0; j < numberOfElectionCommittees; j++){
                if(i==0){
                    ...
                    mainReportTable[j][i] = String.format(Locale.US,"%.2f",(floatValue / numberOfSeatsToBeDistributed * 100));
                }
            }
        }
    }
    ...
    public static void sortedWorkingTable_NumVotes_and_CommName(){
        String[][] partialArray = new String[numberOfElectionCommittees*numberOfSeatsToBeDistributed][2];
        ...
        String[][] tableOfHighestPartialNumbersWithCommitteeName = new String[numberOfSeatsToBeDistributed][2];
